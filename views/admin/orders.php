        ...
        <div class="admin-card">
            ...
            <div class="admin-table-wrap">
                <table class="admin-table">
                    ...
                    <tr>
                        <td><strong style="color:#2563eb;"><?= htmlspecialchars($o['order_code']) ?></strong></td>
                        <td>
                            <div style="font-weight:600;"><?= htmlspecialchars($o['shipping_name']) ?></div>
                            <div style="font-size:11.5px;color:#8a96b8;"><?= htmlspecialchars($o['shipping_phone']) ?></div>
                        </td>
                        <td style="font-weight:700;color:#dc2626;"><?= formatPrice($o['total_amount']) ?></td>
                        <td>
                            <?php $pm = ['cod'=>'COD','bank'=>'Chuyển khoản','momo'=>'MoMo','vnpay'=>'VNPay'];
                                  echo htmlspecialchars($pm[$o['payment_method']] ?? $o['payment_method']); ?>
                        </td>
                        <td style="font-size:12.5px;color:#8a96b8;"><?= date('d/m/Y H:i', strtotime($o['created_at'])) ?></td>
                        <td>
                            <span class="badge-status" style="background:<?= $statusColors[$o['status']] ?>1a;color:<?= $statusColors[$o['status']] ?>;border-color:<?= $statusColors[$o['status']] ?>40;">
...
function openStatusModal(o) {
    ...
    document.getElementById('statusOrderCode').textContent = 'Đơn #' + o.order_code + ' — ' + o.shipping_name;
    document.getElementById('statusSelect').value = o.status;
    toggleCancelReason(o.status);
