...
=======
                ...
                <div class="pd-gallery__main" id="galleryMain">
                    ...
                    <?php if (!empty($product['is_new'])): ?>
                        <span class="pd-gallery__badge pd-gallery__badge--new">Mới</span>
                    <?php endif; ?>
                    <img src="<?= imgUrl($product['thumbnail']) ?>" alt="<?= htmlspecialchars($product['name']) ?>"
                        id="mainImage" onerror="this.onerror=null; this.src='<?= ASSETS_URL ?>/images/no-image.webp'">
                    <button class="pd-gallery__arrow pd-gallery__arrow--prev" id="galleryPrev" type="button"
                        aria-label="Ảnh trước">
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    <button class="pd-gallery__arrow pd-gallery__arrow--next" id="galleryNext" type="button"
                        aria-label="Ảnh sau">
                        <i class="fas fa-chevron-right"></i>
                    </button>
>>>>>>> 1a0162bf779ee750c081ca565ec555db7b89514e
                ...
                <div class="pd-gallery__thumbs" id="thumbStrip">
                    <!-- Tile 0 — thumbnail (always shown) -->
                    <button class="pd-gallery__thumb active" data-src="<?= imgUrl($product['thumbnail']) ?>"
                        type="button" aria-label="View 1">
                    ...
                    foreach ($images as $img):
                        if ($shown >= 3)
                            break;
                        $shown++; ?>
                        <button class="pd-gallery__thumb" data-src="<?= imgUrl($img['image_path']) ?>" type="button"
                            aria-label="View <?= $shown + 1 ?>">
...
>>>>>>> 1a0162bf779ee750c081ca565ec555db7b89514e
            ...
            <section class="pd-info">
                ...
                <div class="pd-info__meta">
                    ...
                    <?php if (!empty($product['sold_quantity'])): ?>
                        <span class="pd-info__sep">|</span>
                        <span class="pd-info__sold"><i class="fas fa-fire"></i> Đã bán
                            <?= number_format($product['sold_quantity']) ?></span>
                    <?php endif; ?>
                </div>
                                ...
                                <div class="pd-config__chips" data-group="ram">
                                    <?php foreach ($ramOptions as $i => $ram): ?>
                                        <button class="pd-chip<?= $i === 0 ? ' is-active' : '' ?>"
                                            type="button"><?= htmlspecialchars($ram) ?></button>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        <?php endif; ?>
                            ...
                            <div class="pd-config__row">
                                ...
                                <div class="pd-config__chips" data-group="storage">
                                    <?php foreach ($storageOptions as $i => $st): ?>
                                        <button class="pd-chip<?= $i === 0 ? ' is-active' : '' ?>"
                                            type="button"><?= htmlspecialchars($st) ?></button>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="pd-cv__list">
                        ...
                        <div class="pd-cv__card is-active">
                            ...
                            <div class="pd-cv__info">
                                <span class="pd-cv__name"
                                    id="selectedColorName"><?= htmlspecialchars($currentColor) ?></span>
                        ...
                        <?php foreach ($colorVariants as $cv): ?>
                            ...
                            ?>
                            <a class="pd-cv__card" href="<?= SITE_URL ?>/product/<?= htmlspecialchars($cv['slug']) ?>">
                                <div class="pd-cv__img">
                                    <img src="<?= imgUrl($cv['thumbnail']) ?>" alt="<?= htmlspecialchars($cvColor) ?>"
                                        onerror="this.onerror=null; this.src='<?= ASSETS_URL ?>/images/no-image.webp'">
                                </div>
                                <div class="pd-cv__info">
                ...
                <?php if ($product['stock'] > 0): ?>
                    ...
                    <div class="pd-qty">
                        ...
                        <div class="pd-qty__ctrl">
                            ...
                            <input class="pd-qty__input" type="number" id="qtyInput" value="1" min="1"
    ...
    ====================================================================== -->
        <div class="pd-tabs" id="pd-tabs">
            <nav class="pd-tabs__nav">
                <button class="pd-tabs__btn is-active" data-tab="specs" type="button"><i class="fas fa-microchip"></i>
                    Thông số</button>
                <button class="pd-tabs__btn" data-tab="desc" type="button"><i class="fas fa-align-left"></i> Mô
                    tả</button>
                <button class="pd-tabs__btn" data-tab="reviews" type="button" id="pd-review-tab"><i
                        class="fas fa-star"></i> Đánh giá (<?= count($reviews) ?>)</button>
                <button class="pd-tabs__btn" data-tab="comments" type="button"><i class="fas fa-comments"></i> Hỏi đáp
            ...
            <div class="pd-tabs__body">
                    ...
                    <?php if (!empty($specs)): ?>
                        <table class="pd-specs">
                            <?php foreach ($specs as $key => $value): ?>
                                <tr>
                                    <td class="pd-specs__key">
                                        <?= htmlspecialchars(is_array($value) ? ($value['key'] ?? $key) : $key) ?></td>
                                    <td class="pd-specs__val">
                                        <?= htmlspecialchars(is_array($value) ? ($value['value'] ?? '') : $value) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </table>
                    <?php else: ?>
                ...
                <div class="pd-tabs__panel" id="tab-desc">
                    <div class="pd-desc">
                        <?= nl2br(htmlspecialchars($product['description'] ?? 'Chưa có mô tả chi tiết.')) ?></div>
                </div>
                                        ...
                                        <div class="pd-replies">
                                            <?php foreach ($comment['replies'] as $reply): ?>
                                                <div class="pd-reply">
                                                    ...
                                                    <div>
                                                        <strong
                                                            class="pd-comment__name"><?= htmlspecialchars($reply['full_name']) ?></strong>
