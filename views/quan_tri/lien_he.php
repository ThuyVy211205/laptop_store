    ...
    <div class="admin-main">
        ...
        <header class="admin-topbar">
            ...
            <div class="admin-topbar-right">
                <span class="admin-user"><i class="fas fa-user-circle me-1"></i><?= htmlspecialchars($admin['ho_ten']) ?></span>
            </div>
        </header>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <?php foreach ($contacts as $c): ?>
                            <tr class="<?= $c['trang_thai'] === 'new' ? 'table-warning' : '' ?>">
                                <td class="text-muted" style="font-size:12px">#<?= $c['id'] ?></td>
                                <td>
                                    <div class="fw-600"><?= htmlspecialchars($c['ten']) ?></div>
                                    <div class="text-muted" style="font-size:12px"><?= htmlspecialchars($c['thu_dien_tu']) ?></div>
                                    <?php if ($c['so_dien_thoai']): ?>
                                    <div class="text-muted" style="font-size:12px"><i class="fas fa-phone fa-xs me-1"></i><?= htmlspecialchars($c['so_dien_thoai']) ?></div>
                                    <?php endif; ?>
                                    <?php if ($c['id_don_hang']): ?>
                                    <div style="font-size:11px;color:#6366f1">Đơn: <?= htmlspecialchars($c['id_don_hang']) ?></div>
                                    <?php endif; ?>
                                </td>
                                <td style="max-width:160px">
                                    <?php $subjectDisplay = $subjectMap[$c['chu_de']] ?? $c['chu_de'] ?? ''; ?>
                                ...
                                <td style="max-width:220px">
                                    <span class="text-truncate d-block" style="max-width:210px" title="<?= htmlspecialchars($c['noi_dung']) ?>">
                                        <?= htmlspecialchars(mb_substr($c['noi_dung'], 0, 80)) ?><?= mb_strlen($c['noi_dung']) > 80 ? '…' : '' ?>
                                    </span>
                                    <?php if ($c['noi_dung_tra_loi']): ?>
                                    <div class="mt-1" style="font-size:11px;color:#16a34a">
                                        <i class="fas fa-reply fa-xs me-1"></i>
                                        Phản hồi bởi <?= htmlspecialchars($c['nguoi_tra_loi']) ?>
                                    </div>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <span class="badge bg-<?= $statusClass[$c['trang_thai']] ?? 'secondary' ?>">
                                        <?= $statusLabel[$c['trang_thai']] ?? $c['trang_thai'] ?>
                                    </span>
                                </td>
                                <td style="font-size:12px;color:#64748b;white-space:nowrap">
                                    <?= date('d/m/Y H:i', strtotime($c['ngay_tao'])) ?>
                                </td>
                                <td>
                                    <div class="d-flex gap-1">
                                        <!-- Nút xem & phản hồi -->
                                        <button type="button" class="btn btn-sm btn-primary" title="Xem & phản hồi"
                                                data-contact="<?= htmlspecialchars(json_encode($c), ENT_QUOTES) ?>"
                                                onclick="openReply(JSON.parse(this.dataset.contact))">
                                            <i class="fas fa-reply"></i>
                                        </button>
                                        <?php if ($c['trang_thai'] === 'new'): ?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <!-- Form phản hồi -->
                <form method="POST" action="<?= SITE_URL ?>/admin/contacts" id="replyForm">
                    <input type="hidden" name="reply_id" id="rId">
                    <div class="mb-3">
...
function openReply(data) {
    ...
    document.getElementById('rName').textContent    = data.ten;
    document.getElementById('rEmail').textContent   = data.thu_dien_tu;
    document.getElementById('rPhone').textContent   = data.so_dien_thoai || '—';
    document.getElementById('rOrderId').textContent = data.id_don_hang   || '—';
    document.getElementById('rSubject').textContent = data.chu_de        || '—';
    document.getElementById('rMessage').textContent = data.noi_dung;

    const replyBlock = document.getElementById('rReplyBlock');
    if (data.noi_dung_tra_loi) {
        document.getElementById('rReplyContent').textContent = data.noi_dung_tra_loi;
        document.getElementById('rRepliedBy').textContent    =
            'Phản hồi bởi ' + data.nguoi_tra_loi + ' — ' + (data.ngay_tra_loi || '');
