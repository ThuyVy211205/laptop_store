...
?>
<?php
$finalPrice = $product['gia_khuyen_mai'] ?: $product['gia'];
$discount   = $product['gia_khuyen_mai'] ? calcDiscount($product['gia'], $product['gia_khuyen_mai']) : 0;
...
<!-- Breadcrumb -->
<?php
$_catName = strtolower(($product['ten_danh_muc'] ?? '') . ' ' . ($product['duong_dan_danh_muc'] ?? ''));
...
<nav class="pd-breadcrumb-bar">
    <div class="container">
        <ol class="breadcrumb mb-0">
            ...
            <?php endif; ?>
            <?php if (!empty($product['duong_dan_danh_muc'])): ?>
            <li class="breadcrumb-item">
                <a href="<?= SITE_URL ?>/category/<?= htmlspecialchars($product['duong_dan_danh_muc']) ?>">
                    <?= htmlspecialchars($product['ten_danh_muc']) ?>
                </a>
            </li>
            <?php endif; ?>
            <li class="breadcrumb-item active"><?= htmlspecialchars(truncate($product['ten'], 55)) ?></li>
        </ol>
    </div>
</nav>
    ...
    <div class="pd-grid">
        ...
        <section class="pd-gallery">
            ...
            <div class="pd-gallery__main" id="galleryMain">
                ...
                <?php endif; ?>
                <img
                    src="<?= imgUrl($product['hinh_thu_nho']) ?>"
            ...
            <div class="pd-gallery__thumbs" id="thumbStrip">
                ...
                <button class="pd-gallery__thumb active" data-src="<?= imgUrl($product['hinh_thu_nho']) ?>" type="button" aria-label="View 1">
                    <img src="<?= imgUrl($product['hinh_thu_nho']) ?>" alt="" onerror="this.onerror=null;this.src='<?= noImageUrl() ?>'">
                </button>
                <?php $shown = 0; foreach ($images as $img): if ($shown >= 3) break; $shown++; ?>
                <button class="pd-gallery__thumb" data-src="<?= imgUrl($img['duong_dan_anh']) ?>" type="button" aria-label="View <?= $shown + 1 ?>">
                    <img src="<?= imgUrl($img['duong_dan_anh']) ?>" alt="" onerror="this.onerror=null;this.src='<?= noImageUrl() ?>'">
                </button>
                <?php endforeach; ?>
                <?php /* Pad to always show 4 thumbnails */
                for ($p = 1 + $shown; $p < 4; $p++): ?>
                <button class="pd-gallery__thumb" data-src="<?= imgUrl($product['hinh_thu_nho']) ?>" type="button">
                    <img src="<?= imgUrl($product['hinh_thu_nho']) ?>" alt="" onerror="this.onerror=null;this.src='<?= noImageUrl() ?>'">
                </button>
                <?php endfor; ?>
            </div>
        ...
        <section class="pd-info">
            ...
            <div class="pd-info__toprow">
                <?php if (!empty($product['thuong_hieu'])): ?>
                <span class="pd-info__brand"><?= htmlspecialchars($product['thuong_hieu']) ?></span>
                <?php endif; ?>
            ...
            <!-- Title -->
            <h1 class="pd-info__title"><?= htmlspecialchars($product['ten']) ?></h1>

            <!-- Stars + review link + sold -->
            <div class="pd-info__meta">
                <?php $ra = (float)($product['diem_danh_gia'] ?? 0); ?>
                ...
                <?php endif; ?>
                <?php if (!empty($product['so_danh_gia'])): ?>
                <a href="#pd-reviews" class="pd-info__review-link" id="scrollToReviews">
                    (<?= number_format($product['so_danh_gia']) ?> đánh giá)
                </a>
                <?php else: ?>
                <a href="#pd-reviews" class="pd-info__review-link" id="scrollToReviews">Viết đánh giá đầu tiên</a>
                <?php endif; ?>
                <?php if (!empty($product['so_luong_ban'])): ?>
                <span class="pd-info__sep">|</span>
                <span class="pd-info__sold"><i class="fas fa-fire"></i> Đã bán <?= number_format($product['so_luong_ban']) ?></span>
                <?php endif; ?>
            </div>
            ...
            <div class="pd-price">
                ...
                <div class="pd-price__original-row">
                    <span class="pd-price__original"><?= formatPrice($product['gia']) ?></span>
            ...
            if (empty($currentColor)) {
                $catSlug   = strtolower($product['duong_dan_danh_muc'] ?? '');
                $brandName = strtolower($product['thuong_hieu'] ?? '');
            ...
            <div class="pd-cv">
                ...
                <div class="pd-cv__list">
                    ...
                    <?php
                    $cvFinal = $cv['gia_khuyen_mai'] ?: $cv['gia'];
                    $cvColor = trim($cv['color'] ?? '');
                    if (empty($cvColor)) {
                        $cvCatSlug   = strtolower($cv['duong_dan_danh_muc'] ?? $product['duong_dan_danh_muc'] ?? '');
                        $cvBrandName = strtolower($cv['thuong_hieu'] ?? '');
                    ...
                    ?>
                    <a class="pd-cv__card"
                       href="<?= SITE_URL ?>/product/<?= htmlspecialchars($cv['duong_dan']) ?>">
                        <div class="pd-cv__img">
                            <img src="<?= imgUrl($cv['hinh_thu_nho']) ?>"
            ...
            <?php
            $catSlugLower = strtolower($product['duong_dan_danh_muc'] ?? '');
    ...
    <div class="pd-tabs" id="pd-tabs">
        ...
        <div class="pd-tabs__body">
            ...
            <div class="pd-tabs__panel" id="tab-desc">
                <div class="pd-desc"><?= nl2br(htmlspecialchars($product['mo_ta'] ?? 'Chưa có mô tả chi tiết.')) ?></div>
            </div>
                ...
                <div class="pd-review-list">
                    ...
                    <div class="pd-review">
                        <div class="pd-review__avatar">
                            ...
                            <img src="<?= imgUrl($review['anh_dai_dien']) ?>" alt="">
                            <?php else: ?>
                            <span><?= mb_strtoupper(mb_substr($review['ho_ten'], 0, 1)) ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="pd-review__body">
                            <strong class="pd-review__name"><?= htmlspecialchars($review['ho_ten']) ?></strong>
                            <?= starRating($review['diem_so']) ?>
                            <p class="pd-review__text"><?= nl2br(htmlspecialchars($review['noi_dung'])) ?></p>
                            <time class="pd-review__time"><?= timeAgo($review['ngay_tao']) ?></time>
                        </div>
                    </div>
                    <?php endforeach; ?>
                ...
                <div class="pd-comment-list">
                    ...
                    <div class="pd-comment">
                        <div class="pd-comment__avatar">
                            ...
                            <img src="<?= imgUrl($comment['anh_dai_dien']) ?>" alt="">
                            <?php else: ?>
                            <span><?= mb_strtoupper(mb_substr($comment['ho_ten'], 0, 1)) ?></span>
                            <?php endif; ?>
                        </div>
                        <div class="pd-comment__body">
                            <strong class="pd-comment__name"><?= htmlspecialchars($comment['ho_ten']) ?></strong>
                            <p class="mb-1"><?= nl2br(htmlspecialchars($comment['noi_dung'])) ?></p>
                            <time class="pd-review__time"><?= timeAgo($comment['ngay_tao']) ?></time>
                            ...
                            <div class="pd-replies">
                                ...
                                <div class="pd-reply">
                                    <div class="pd-comment__avatar pd-comment__avatar--sm">
                                        <span><?= mb_strtoupper(mb_substr($reply['ho_ten'], 0, 1)) ?></span>
                                    </div>
                                    <div>
                                        <strong class="pd-comment__name"><?= htmlspecialchars($reply['ho_ten']) ?></strong>
                                        <p class="mb-1"><?= nl2br(htmlspecialchars($reply['noi_dung'])) ?></p>
                                        <time class="pd-review__time"><?= timeAgo($reply['ngay_tao']) ?></time>
                                    </div>
                                </div>
                                <?php endforeach; ?>
                            </div>
