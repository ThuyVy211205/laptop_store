        ...
        <div class="col-lg-8">
            <div class="cart-items-list" id="cartItemsList">
                <?php foreach ($items as $item):
                    $itemId   = $item['id_san_pham'] ?? $item['id'];
                    $price    = $item['gia_khuyen_mai'] ?: $item['gia'];
                    $subtotal = $price * $item['so_luong'];
                    $hasDiscount = $item['gia_khuyen_mai'] && $item['gia_khuyen_mai'] < $item['gia'];
                    $lowStock = $item['ton_kho'] <= 5;
                ?>
                <div class="cart-item-card" data-id="<?= $itemId ?>" data-price="<?= $price ?>">
                    <a href="<?= SITE_URL ?>/product/<?= htmlspecialchars($item['duong_dan']) ?>" class="cart-item-img">
                        <img src="<?= imgUrl($item['hinh_thu_nho']) ?>" alt="<?= htmlspecialchars($item['ten']) ?>"
                             onerror="this.onerror=null; this.src='<?= ASSETS_URL ?>/images/no-image.webp'">
                    </a>

                    <div class="cart-item-info">
                        <a href="<?= SITE_URL ?>/product/<?= htmlspecialchars($item['duong_dan']) ?>" class="cart-item-name">
                            <?= htmlspecialchars($item['ten']) ?>
                        </a>
                        <div class="cart-item-meta">
                            <span class="cart-item-price"><?= formatPrice($price) ?></span>
                            <?php if ($hasDiscount): ?>
                            <span class="cart-item-price-old"><?= formatPrice($item['gia']) ?></span>
                            <span class="cart-item-discount-tag">
                                -<?= calcDiscount($item['gia'], $item['gia_khuyen_mai']) ?>%
                            </span>
                            <?php endif; ?>
                        </div>
                        <span class="cart-item-stock<?= $lowStock ? ' low' : '' ?>">
                            <i class="fas fa-<?= $lowStock ? 'exclamation-triangle' : 'box' ?> me-1"></i>
                            Còn <?= $item['ton_kho'] ?> sản phẩm<?= $lowStock ? ' — sắp hết!' : '' ?>
                        </span>
                    </div>
                    ...
                    <div class="cart-item-controls">
                        <div class="cart-qty-wrap">
                            ...
                            <input type="number" class="qty-input" data-id="<?= $itemId ?>"
                                   value="<?= $item['so_luong'] ?>" min="1" max="<?= $item['ton_kho'] ?>">
